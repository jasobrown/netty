 ...
 */
final class OutboundOnlyEpollEventLoop extends SingleThreadEventLoop {
    private static final InternalLogger logger = InternalLoggerFactory.getInstance(OutboundOnlyEpollEventLoop.class);
    private static final AtomicIntegerFieldUpdater<OutboundOnlyEpollEventLoop> WAKEN_UP_UPDATER;

    static {
        AtomicIntegerFieldUpdater<OutboundOnlyEpollEventLoop> updater =
        PlatformDependent.newAtomicIntegerFieldUpdater(OutboundOnlyEpollEventLoop.class, "wakenUp");
        if (updater == null) {
            updater = AtomicIntegerFieldUpdater.newUpdater(OutboundOnlyEpollEventLoop.class, "wakenUp");
        }
    ...
    private final Callable<Integer> pendingTasksCallable = new Callable<Integer>() {
        ...
        public Integer call() throws Exception {
            return OutboundOnlyEpollEventLoop.super.pendingTasks();
        }
    };
    private volatile int wakenUp;
    private volatile Thread epollThread;
    private volatile int ioRatio = 50;

    OutboundOnlyEpollEventLoop(EventLoopGroup parent, Executor executor, int maxEvents,
...
//            long timeoutMillis = (selectDeadLineNanos - currentTimeNanos + 500000L) / 1000000L;
            ...
            if (timeoutNanos <= 0) {
                if (selectCnt == 0) {
                    // JEB: we'll only really get here if there's a scheduled task that needs to be executed ASAP
            ...
            int selectedKeys = Native.epollWait(epollFd.intValue(), events, 0);
            selectCnt ++;
